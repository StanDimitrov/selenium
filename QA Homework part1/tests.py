    ...
    for record in resp.json()['category']:
        if record['id'] == 1:
            assert record['name'] == "doggie",\
                "Data matched! Expected : doggie, found : " + str(record['name'])
            assert record['status'] == "pending",\
                "Data matched! Expected : pending, but found : " + str(record['status'])
...
def test_api_post():
    ...
    assert data['name'] == "Sparky", "Pet created with correct name. \
        Expected : Sparky, found : " + str(data['name'])
    assert data['bread'] == "chihuahua", "pet created with correct bread. \
